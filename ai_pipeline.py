...
def generate_answer_with_llm(user_query: str, context_list: list[dict]) -> Optional[str]:
    ...
    context_str = ""
    sources = set()
    sources.add(context_list[0]['source'].replace(
        '_', ' ') + f' Страница №{context_list[0]["page"]}')
    ...
    prompt = f"""<start_of_turn>user
        ...
        5.  В конце ответа **обязательно укажи ИСТОЧНИК(И)** информации. Используй названия документов, указанные в начале каждого фрагмента КОНТЕКСТА. Если можешь определить номер страницы, укажи и его. Пример формата: "(Источник: 'Название документа', стр. 5). Пиши источники каждый на новой строчке".
        ...
        7 . МЧД ЭТО Машиночитаемая доверенность (МЧД) 
...
def generate_live_response_with_llm(user_query: str, query_category: str) -> Optional[str]:
    ...
    prompt = f"""<start_of_turn>user
    ТВОЯ РОЛЬ: {system_prompt}

    ТВОЯ ЗАДАЧА: Внимательно прочитай СООБЩЕНИЕ ПОЛЬЗОВАТЕЛЯ ниже. Следуя ИНСТРУКЦИИ ДЛЯ ТЕБЯ, напиши короткий (до {MAX_NEW_TOKENS_LIVE} токенов), понятный и вежливый ответ **от своего имени (как ИИ-ассистент)**. Не повторяй сообщение пользователя и не пиши ничего от его лица. НЕ ИСПОЛЬЗУЙ СКОБКИ ДЛЯ HTML РАЗМЕТКИ

    СООБЩЕНИЕ ПОЛЬЗОВАТЕЛЯ, НА КОТОРОЕ НУЖНО ОТРЕАГИРОВАТЬ:
    {user_query}
    <end_of_turn>
    <start_of_turn>model
    """
