...
def generate_live_response_with_llm(user_query: str, query_category: str) -> Optional[str]:
    ...
    ТВОЯ ЗАДАЧА: Внимательно прочитай СООБЩЕНИЕ ПОЛЬЗОВАТЕЛЯ ниже. Следуя ИНСТРУКЦИИ ДЛЯ ТЕБЯ, напиши короткий (до {MAX_NEW_TOKENS_LIVE} токенов), понятный и вежливый ответ **от своего имени (как ИИ-ассистент)**. Не повторяй сообщение пользователя и не пиши ничего от его лица.

    ИНСТРУКЦИЯ ДЛЯ ТЕБЯ (АССИСТЕНТА):
    {instruction}
